...
import static de.blankedv.javapanel.ActivePanelElement.*;  // states for sensors etc
...
public class Route {
    ...
    public Route(int id, int btn1, int btn2, String route, String allSensors,
        ...
        if (DEBUG) {
            System.out.println(TAG + " creating route id=" + id);
        }
        ...
        if (DEBUG) {
            System.out.println(TAG + " " + rtSignals.size() + " signals");
        }
        if (DEBUG) {
            System.out.println(TAG + " " + rtTurnouts.size() + " turnouts");
        }
        ...
        if (DEBUG) {
            System.out.println(TAG + " " + rtSensors.size() + " sensors");
        }
    ...
    public void clear() {
        ...
        // automatically
        if (DEBUG) {
            System.out.println(TAG + " clearing route id=" + id);
        }
    ...
    public boolean offendingRouteActive() {
        if (DEBUG) {
            System.out.println(TAG + " checking for (active) offending Routes");
        }
        String[] offRoutes = offendingString.split(",");
        for (int i = 0; i < offRoutes.length; i++) {
            for (Route rt : routes) {
    ...
    public void set() {
        ...
        if (DEBUG) {
            System.out.println(TAG + " setting route id=" + id);
        }
        
        if (offendingRouteActive()) {
            if (DEBUG) System.out.println(TAG + " offending route active");
            return;
        }
        ...
        for (RouteSignal rs : rtSignals) {
            ...
            if (DEBUG) {
                System.out.println(TAG + " setting route signal " + cmd);
            }
            sendQ.add(cmd);
        ...
        for (RouteSignal rs : rtSignals) {
            if (rs.depFrom != INVALID_INT) {
                if (rs.signal.getState() != rs.dynamicValueToSetForRoute()) {
                    ...
                    if (DEBUG) {
                        System.out.println(TAG + " setting route signal dep.(" + rs.depFrom + ") " + cmd);
                    }
                    sendQ.add(cmd);
                }
            }
